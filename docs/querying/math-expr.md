...
| array_prepend(expr,arr) | adds expr to arr at the beginning, the resulting array type determined by the type of the array. If arr is `NULL ARRAY` then the resulting array type is determined by `expr`                                 |
| array_append(arr,expr) | appends expr to arr, the resulting array type determined by the type of the array. If arr is `NULL ARRAY` then the resulting array type is determined by `expr`                                                                                                                     |
